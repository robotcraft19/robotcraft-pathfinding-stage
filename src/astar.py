...
class Node:
    ...
    def calculate_heuristic(self):
        ...
        if complex_a == False:  # only for directions
            ...
            return abs(self.row - Node.target.row) + abs(
                self.column - Node.target.column
            )
        else:
            # return euclidian distance
            return (
                (self.row - Node.target.row) ** 2
                + (self.column - Node.target.column) ** 2
            ) ** 0.5
        ...
        for dir in Node.directions:
            try:
                ...
                for dir_dir in Node.directions:
                    try:
                        nb_nb = Node.matrix[self.row + dir[0] + dir_dir[0]][
                            self.column + dir[1] + dir_dir[1]
                        ]
                        if nb_nb == 1:
                            wall_penalty += 5.0
                    except:
    ...
    def calculate_path(self):
        while len(self.open_nodes) > 0:
            ...
            if current_node == self.target:
                ...
                neighbors = [
                    Node(
                        current_node.row + dir[0],
                        current_node.column + dir[1],
                        current_node.g + dir[2],
                        current_node,
                    )
